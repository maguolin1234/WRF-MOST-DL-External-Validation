#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Xilinhaote Code1 generalization application
=============================================

Applies saved 10-m Xilinhaote Code1 models without retraining.
...
- NO2_1107 / N2
- NO3_1071 / N3
- NOT_1166 / NT (no observed TKE target)

The saved Code1 models use the physical 10-m WS/WD/TKE/T bundle. Height-transfer
tasks vary only the WS/U/V/wind-MOST height; WD, TKE and temperature remain at
the target tower's native 10-m bundle. The resulting multi-height WS display is
therefore a height-wise pseudo-profile rather than a jointly predicted profile.

The script evaluates C2/C4/C6/C8 for CNN_LSTM and TCN, injects both the
Gryning_fixed_z0_vegetation500 wind reference and MOST_T_free_inv_z0h_theta
thermal diagnostic reference used during training, and exports predictions,
metrics, audits, time series and pseudo-profiles.

In addition to source-test-endpoint evaluation, cross_tower_other_time uses all
valid target-tower endpoints. Calendar overlap with the source development
period is retained by design, so this is all-target-time coverage rather than
strict non-overlapping temporal extrapolation.
"""
...
CODE1_RESULT_ROOT = Path(
    r"I:\wake\WRF_RANS_wake\xilinhaote\result\bias_correction\1km\correction\c1"
)
OUTPUT_ROOT = Path(
...
def inject_diagnostic_references(
    ...
    """Inject Xilinhaote wind and thermal diagnostic profiles at available heights."""
    out = df.copy()
    report: Dict[str, Any] = {
        "site": site,
    ...
    # Wind-speed diagnostic reference.
    try:
        u_table = resolve_diag_long_csv("U")
        report["u_csv"] = str(u_table)
        gu = pd.read_csv(u_table)
        gu["time"] = parse_datetime_mixed_safe(gu["time"], "time")
        gu["height_m"] = numeric(gu["height_m"])
        gu["pred"] = numeric(gu["pred"])
        mask = gu["method"].astype(str).map(norm).eq(norm(DIAG_U_METHOD))
        if "dataset_version" in gu.columns:
            ds = gu["dataset_version"].astype(str).map(norm)
            req = norm(DIAG_DATASET_VERSION)
            mask &= ds.eq(req) | ds.str.contains(req, na=False)
        available_site_cols = [c for c in site_cols if c in gu.columns]
        if available_site_cols:
            sm = np.zeros(len(gu), dtype=bool)
            for c in available_site_cols:
                sm |= gu[c].astype(str).str.contains(
                    site, case=False, na=False, regex=False
                ).to_numpy()
            mask &= sm
        gus = gu.loc[mask].dropna(subset=["time", "height_m", "pred"]).copy()
        for hval, gh in gus.groupby("height_m"):
    ...
    # Thermal diagnostic reference.
    try:
        t_table = resolve_diag_long_csv("T")
        report["t_csv"] = str(t_table)
        gt = pd.read_csv(t_table)
        gt["time"] = parse_datetime_mixed_safe(gt["time"], "time")
        gt["height_m"] = numeric(gt["height_m"])
        gt["pred"] = numeric(gt["pred"])
        mask = gt["method"].astype(str).map(norm).eq(norm(DIAG_T_METHOD))
        if "dataset_version" in gt.columns:
            ds = gt["dataset_version"].astype(str).map(norm)
            req = norm(DIAG_DATASET_VERSION)
            mask &= ds.eq(req) | ds.str.contains(req, na=False)
        available_site_cols = [c for c in site_cols if c in gt.columns]
        if available_site_cols:
            sm = np.zeros(len(gt), dtype=bool)
            for c in available_site_cols:
                sm |= gt[c].astype(str).str.contains(
                    site, case=False, na=False, regex=False
                ).to_numpy()
            mask &= sm
        gts = gt.loc[mask].dropna(subset=["time", "height_m", "pred"]).copy()
        for hval, gh in gts.groupby("height_m"):
            if not np.isfinite(hval):
                continue
            h = int(round(float(hval)))
            ...
            wrf_col = first_existing(out, [
                ...
                f"WRF_T{h} (K)", f"WRF_T_{h}m (K)",
                f"WRF_TEMP{h} (K)", f"WRF_TEMP_{h}m (K)",
                "T2 (K)", "WRF_T2 (K)", "T2", "WRF_T2",
            ])
            if wrf_col is not None:
                out[diff_col] = ref - numeric(out[wrf_col])
...
def alias_target_bundle(df: pd.DataFrame, task: GeneralizationTask) -> Tuple[pd.DataFrame, Dict[str, str]]:
    ...
    # Thermal diagnostic MOST features remain at the target tower's native temperature height.
    alias([f"T_MO{h_t} (K)", f"MOST_T{h_t} (K)", f"T_MO_{h_t}m (K)", f"MOST_T_{h_t}m (K)", f"T_MO{h_t}", f"MOST_T{h_t}"], "T_MO10 (K)")
    alias([f"MOST_T{h_t} (K)", f"T_MO{h_t} (K)", f"MOST_T_{h_t}m (K)", f"T_MO_{h_t}m (K)"], "MOST_T10 (K)")
    alias([f"T_MO_minus_WRF{h_t} (K)", f"T_MO_minus_WRF_{h_t}m (K)", f"MOST_T_minus_WRF{h_t} (K)"], "T_MO_minus_WRF10 (K)")
    alias([f"MOST_T_minus_WRF{h_t} (K)", f"T_MO_minus_WRF{h_t} (K)", f"MOST_T_minus_WRF_{h_t}m (K)"], "MOST_T_minus_WRF10 (K)")
    alias([f"GryningThermal_T_MO{h_t} (K)", f"T_MO{h_t} (K)", f"MOST_T{h_t} (K)"], "GryningThermal_T_MO10 (K)")
...
def generate_tasks(
...
) -> List[GeneralizationTask]:
    """Generate same-tower and all ordered cross-tower tasks for four sites."""
    ...
    def make(
        ...
        ws_h: int,
        gtype: str,
        time_mode: str = "source_test_endpoints",
    ) -> GeneralizationTask:
    ...
    # Vertical transfer within the source tower.
    if RUN_SAME_TOWER_OTHER_HEIGHT and source_site in target_data:
        for h in available_obs_ws_heights(target_data[source_site]):
            if h != source_h:
                tasks.append(make(source_site, h, "same_tower_other_height"))

    # Ordered cross-tower transfer: every source tower -> each of the other towers.
    for target_site in TARGET_SITES:
        if target_site == source_site or target_site not in target_data:
            continue
        target_heights = available_obs_ws_heights(target_data[target_site])
        if RUN_CROSS_TOWER_SAME_HEIGHT and source_h in target_heights:
            tasks.append(make(target_site, source_h, "cross_tower_same_height"))
        if RUN_CROSS_TOWER_OTHER_HEIGHT:
            for h in target_heights:
                if h != source_h:
                    tasks.append(make(target_site, h, "cross_tower_other_height"))
...
def apply_model_task(
    ...
    df_gryn, gryning_report = inject_diagnostic_references(
    ...
    audit = {
        ...
        "task": task.__dict__,
        "alias_map": alias_map,
        "diagnostic_references": gryning_report,
...
def main() -> None:
        ...
        for task in tasks_by_source[artifact.source_site]:
            ...
            try:
                ...
                if not pred.empty:
                    ...
                    if SAVE_PLOTS:
                        for var, h in [("WS", task.apply_ws_height_m), ("WD", task.apply_wd_height_m), ("TKE", task.apply_tke_height_m), ("T", task.apply_temp_height_m)]:
    ...
    summary = {
        ...
        "note": "Four-site Xilinhaote transfer. WS height is varied; WD/TKE/T remain at target-native 10 m. NOT_1166 has no observed TKE, so corresponding TKE metrics may have N=0.",
    }
