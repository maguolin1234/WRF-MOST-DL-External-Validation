#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Xilinhaote Code2 profile-model generalization application
===========================================================

Applies saved multi-height Xilinhaote Code2 models without retraining for the
four towers NO1_1159, NO2_1107, NO3_1071 and NOT_1166.

Every ordered source-target tower pair is evaluated. The formal cases are C2,
C4b, C6m and C8d for CNN_LSTM and TCN. Saved profile models use the configured
4, 10, 30, 50, 70 and 100 m structure; WD heads at 4 and 50 m are absent by the
training drop rule. NOT_1166 has no observed TKE, so TKE outputs can be
reconstructed but target-side TKE metrics may have N=0.

The script uses the original 10-min ML-ready inputs, matching the supplied
Code2 trainer. It injects the D1H Gryning_fixed_z0_vegetation500 wind references
and MOST_T_free_inv_z0h_theta thermal diagnostic references, reproducing the
trainer wrapper's default diagnostic-resolution behavior. Missing model-input
WRF/MOST heights may be interpolated from bracketing predictor levels, but
target-tower observations are never interpolated.

Both source-test common-time and all-valid-target-time tasks are generated.
Calendar overlap with source development periods is retained in the latter.

Saved WD heads remain at their physical heights whenever matching target OBS/WRF
exist; only a non-evaluable WD head is rebased to the target's native 10-m WD
baseline to support matched wind-vector and wind-rose-ready diagnostics.
"""
...
TIME_RESOLUTION_MODE = "original"  # matches the supplied Code2 trainer (four_sites_full, 10-min)
TIME_RESOLUTION_DATA_ROOT = Path(
    r"I:\wake\WRF_RANS_wake\xilinhaote\result\bias_correction\1km\inputdata\data_post_new\time_resolution_sensitivity"
)
DATASET_ROOTS: Dict[str, Path] = {
    "10min": TIME_RESOLUTION_DATA_ROOT / "D10_10min",
    "1h": TIME_RESOLUTION_DATA_ROOT / "D1H_1hour",
    "original": Path(
        r"I:\wake\WRF_RANS_wake\xilinhaote\result\bias_correction\1km\inputdata\data_post_new\four_sites_full"
    ),
}
ML_READY_ROOT = DATASET_ROOTS[TIME_RESOLUTION_MODE]
CODE2_RESULT_ROOT = Path(
    r"I:\wake\WRF_RANS_wake\xilinhaote\result\bias_correction\1km\correction\c2"
)
OUTPUT_ROOT = Path(
...
DIAG_REFERENCE_ROOT = Path(
    r"I:\wake\WRF_RANS_wake\xilinhaote\result\bias_correction\1km\inputdata\data_post_new\gryning_diagnostics"
)
DIAG_U_LONG_CSV = DIAG_REFERENCE_ROOT / "20_U_diagnostics" / "22_U_timeseries_long_all_methods_all_files.csv"
DIAG_T_LONG_CSV = DIAG_REFERENCE_ROOT / "30_T_diagnostics" / "32_T_timeseries_long_all_methods_all_files.csv"
DIAG_SEARCH_ROOT = TIME_RESOLUTION_DATA_ROOT.parent
DIAG_AUTO_DISCOVER = True
DIAG_U_METHOD = "Gryning_fixed_z0_vegetation500"
DIAG_T_METHOD = "MOST_T_free_inv_z0h_theta"
# The supplied Xilinhaote Code2 wrapper does not define TIME_RESOLUTION_MODE;
# its diagnostic helper therefore defaults to D1H even though model inputs come
# from the original 10-min ML-ready files. Preserve that exact training behavior.
DIAG_DATASET_VERSION = "D1H_1hour_aggregated"
DIAG_MIN_VALID_RATIO = 0.10  # D1H references merged into original 10-min inputs

TIME_COL = "北京时间"
OBS_VALID_COL = "obs_valid_flag"
CASES_TO_APPLY = ["C2", "C4b", "C6m", "C8d"]
NETWORKS_TO_APPLY = ["CNN_LSTM", "TCN"]
SOURCE_SITES = ["NO1_1159", "NO2_1107", "NO3_1071", "NOT_1166"]
TARGET_SITES = ["NO1_1159", "NO2_1107", "NO3_1071", "NOT_1166"]
...
MOST_REFERENCE_INTERPRETATION = (
    "OBS-assisted diagnostic Gryning_fixed_z0_vegetation500 wind reference and "
    "MOST_T_free_inv_z0h_theta thermal reference; training-consistent but not strict deployment"
)
ALLOW_EXISTING_MOST_COLUMN_FALLBACK = True

SITE_CONFIG: Dict[str, Dict[str, Any]] = {
    "NO1_1159": {"aliases": ["NO1_1159", "N1"], "preferred_domain": "d03", "native_wd_m": 10, "has_tke": True},
    "NO2_1107": {"aliases": ["NO2_1107", "N2"], "preferred_domain": "d03", "native_wd_m": 10, "has_tke": True},
    "NO3_1071": {"aliases": ["NO3_1071", "N3"], "preferred_domain": "d03", "native_wd_m": 10, "has_tke": True},
    "NOT_1166": {"aliases": ["NOT_1166", "NT"], "preferred_domain": "d03", "native_wd_m": 10, "has_tke": False},
}
...
def path_contains_alias(path: Path, aliases: Sequence[str]) -> bool:
    """Match long station IDs anywhere, but short N1/N2/N3/NT tags only as path parts."""
...
def read_json(path: Path) -> Dict[str, Any]:
    """Read a UTF-8 JSON file used by saved Code2 model artifacts."""
...
_DIAG_RESOLVED_PATHS: Dict[str, Path] = {}


def resolve_diag_long_csv(kind: str) -> Path:
    """Find the schema-compatible Xilinhaote U or T diagnostic long table."""
    kind = str(kind).upper()
    if kind not in {"U", "T"}:
        raise ValueError(f"Unknown diagnostic kind: {kind}")
    if kind in _DIAG_RESOLVED_PATHS:
        return _DIAG_RESOLVED_PATHS[kind]

    configured = DIAG_U_LONG_CSV if kind == "U" else DIAG_T_LONG_CSV
    filename = configured.name
    method = DIAG_U_METHOD if kind == "U" else DIAG_T_METHOD
    candidates: List[Path] = [configured]
    root = Path(DIAG_SEARCH_ROOT)
    if DIAG_AUTO_DISCOVER and root.exists():
        candidates.extend(root.rglob(filename))
        subtoken = "20_u_diagnostics" if kind == "U" else "30_t_diagnostics"
        for p in root.rglob("*.csv"):
            low = str(p).lower()
            if "gryning" in low and subtoken in low:
                candidates.append(p)
    ...
    if not unique:
        raise FileNotFoundError(
            f"No schema-compatible {kind} diagnostic long table found. "
            f"Configured={configured}; search_root={DIAG_SEARCH_ROOT}"
        )
    ...
    for p in unique:
        try:
            ...
            mask = g["method"].astype(str).map(norm).eq(norm(method))
            if "dataset_version" in g.columns:
                ds = g["dataset_version"].astype(str).map(norm)
                req = norm(DIAG_DATASET_VERSION)
            ...
            for site in SOURCE_SITES:
                ...
                for c in [x for x in ["site_id", "relative_file", "csv_file"] if x in g.columns]:
                    sm |= g[c].astype(str).str.contains(
    ...
    selected = ranked[0][3]
    _DIAG_RESOLVED_PATHS[kind] = selected
    print(
        f"[Diag-{kind}] selected={selected} | "
        f"min_four_site_unique_times={ranked[0][0]} | sum={ranked[0][1]}"
    )
    return selected


def merge_diagnostic_height_by_time(
    left_df: pd.DataFrame,
    right_df: pd.DataFrame,
    value_name: str,
) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """Merge one diagnostic height using exact/floor/round and guarded step shifts."""
    left = left_df.copy()
    right = right_df.copy()
    ...
    def score(a, b) -> int:
        return len(set(pd.Series(a).dropna().unique()).intersection(
            set(pd.Series(b).dropna().unique())
        ))
    ...
    rr = right.groupby("__merge_time", as_index=False)["pred"].mean().rename(
        columns={"pred": value_name}
    )
    ...
    return out, {
        ...
        "diagnostic_time_min": str(rraw.min()) if rraw.notna().any() else None,
        "diagnostic_time_max": str(rraw.max()) if rraw.notna().any() else None,
    }
...
def discover_code2_models() -> List[ModelArtifact]:
    ...
    metadata_paths = list(CODE2_RESULT_ROOT.rglob("metadata.json"))
    print(f"[model discovery] root={CODE2_RESULT_ROOT} | metadata.json={len(metadata_paths)}")
    ...
    for meta_path in metadata_paths:
        ...
        if not model_path.exists() or not scaler_path.exists():
            incomplete_dirs.append(str(model_dir))
            continue
        try:
            meta = read_json(meta_path)
        except Exception as exc:
            json_errors.append(f"{meta_path}: {exc!r}")
            continue
    ...
    print(f"[model discovery] complete_before_filters={len(metadata_paths) - len(incomplete_dirs) - len(json_errors)} | accepted_before_dedup={len(found)} | incomplete={len(incomplete_dirs)} | json_errors={len(json_errors)}")
    if json_errors:
        print("[model discovery] first JSON error:", json_errors[0])
    if incomplete_dirs:
        print("[model discovery] first incomplete model directory:", incomplete_dirs[0])
...
def most_temperature_candidates(h: int) -> List[str]:
    return [
        f"T_MO{h} (K)", f"MOST_T{h} (K)",
        f"T_MO_{h}m (K)", f"MOST_T_{h}m (K)",
        f"T_MO{h}", f"MOST_T{h}",
...
def _create_most_temperature_height(
    df: pd.DataFrame,
    target_h: int,
) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """Create missing diagnostic MOST temperature at target_h from bracketing levels."""
    out = df.copy()
    t_target = f"T_MO{target_h} (K)"
    most_target = f"MOST_T{target_h} (K)"
    diff_target = f"T_MO_minus_WRF{target_h} (K)"
    most_diff_target = f"MOST_T_minus_WRF{target_h} (K)"
    keep_target = f"GryningThermal_T_MO{target_h} (K)"

    direct = first_existing(out, most_temperature_candidates(target_h))
    if direct is not None and numeric(out[direct]).notna().sum() > 0:
        temp = numeric(out[direct])
        out[t_target] = temp
        out[most_target] = temp
        out[keep_target] = temp
        wrf_t = first_existing(out, wrf_temp_candidates(target_h))
        if wrf_t is not None:
            out[diff_target] = temp - numeric(out[wrf_t])
            out[most_diff_target] = out[diff_target]
        return out, {
            "height_m": target_h, "status": "already_available",
            "source_col": direct, "wrf_t_col": wrf_t,
            "uses_target_observations": False,
        }

    available = _extract_available_heights(out.columns, ["TMO", "MOSTT"])
    valid: List[int] = []
    cols: Dict[int, str] = {}
    for h in available:
        col = first_existing(out, most_temperature_candidates(h))
        if col is not None and numeric(out[col]).notna().sum() > 0:
            valid.append(h)
            cols[h] = col
    bracket = _bracketing_heights(valid, target_h)
    if bracket is None:
        return out, {
            "height_m": target_h, "status": "no_bracketing_most_temperature_levels",
            "available_heights": available, "valid_heights": valid,
        }

    h0, h1 = bracket
    alpha = (float(target_h) - float(h0)) / (float(h1) - float(h0))
    t0 = numeric(out[cols[h0]]).to_numpy(dtype=float)
    t1 = numeric(out[cols[h1]]).to_numpy(dtype=float)
    temp = (1.0 - alpha) * t0 + alpha * t1
    out[t_target] = temp
    out[most_target] = temp
    out[keep_target] = temp
    wrf_t = first_existing(out, wrf_temp_candidates(target_h))
    ...
    created = [t_target, most_target, keep_target]
    if wrf_t is not None:
        created.extend([diff_target, most_diff_target])
    return out, {
        "height_m": target_h, "status": "interpolated_from_most_temperature",
        "lower_height_m": h0, "upper_height_m": h1, "alpha": alpha,
        "lower_col": cols[h0], "upper_col": cols[h1],
        "wrf_t_col": wrf_t, "created_columns": created,
        "valid_rows": int(np.isfinite(temp).sum()),
        "uses_target_observations": False,
    }
...
def ensure_model_height_support(
    ...
    """Fill missing saved-model predictor heights without creating target OBS."""
    out = df.copy()
    report: Dict[str, Any] = {
    ...
    wrf_needed = set()
    most_speed_needed = set()
    most_temp_needed = set()

    for h in report["required_heights"]:
        if any(token in feature_text for token in [
            f"WRF_U{h}", f"WRF_V{h}", f"WRF_WS{h}", f"WRF_WD{h}"
        ]):
            wrf_needed.add(h)
        if any(token in feature_text for token in [
            f"U_MO{h}", f"MOST_WS{h}", f"MO_minus_WRF{h}"
        ]):
            most_speed_needed.add(h)
        if any(token in feature_text for token in [
            f"T_MO{h}", f"MOST_T{h}",
            f"T_MO_minus_WRF{h}", f"MOST_T_minus_WRF{h}",
            f"GryningThermal_T_MO{h}",
        ]):
            most_temp_needed.add(h)
    ...
    for h in sorted(most_speed_needed):
        needs_creation = any(
            c not in out.columns or numeric(out[c]).notna().sum() == 0
            for c in [
                f"U_MO{h} (m/s)",
                f"MOST_WS{h} (m/s)",
                f"MO_minus_WRF{h} (m/s)",
            ]
        )
        if needs_creation:
    ...
    for h in sorted(most_temp_needed):
        required_cols = [f"T_MO{h} (K)", f"MOST_T{h} (K)"]
        needs_creation = any(
            c not in out.columns or numeric(out[c]).notna().sum() == 0
            for c in required_cols
        )
        if needs_creation:
            out, r = _create_most_temperature_height(out, h)
            report["most_temperature"].append(r)

    return out, report



def inject_diagnostic_references(
    df: pd.DataFrame,
    site: str,
) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """Inject Xilinhaote wind and thermal diagnostic profiles at all available heights."""
    out = df.copy()
    report: Dict[str, Any] = {
        "site": site,
        "dataset_version": DIAG_DATASET_VERSION,
        "u_method": DIAG_U_METHOD,
        "t_method": DIAG_T_METHOD,
        "interpretation": MOST_REFERENCE_INTERPRETATION,
        "minimum_valid_ratio": DIAG_MIN_VALID_RATIO,
        "u_heights": [],
        "t_heights": [],
    }
    site_cols = ["site_id", "relative_file", "csv_file"]

    def load_site_rows(kind: str, method: str) -> Tuple[pd.DataFrame, Path]:
        table = resolve_diag_long_csv(kind)
        g = pd.read_csv(table)
        g["time"] = parse_datetime_mixed_safe(g["time"], "time")
        g["height_m"] = numeric(g["height_m"])
        g["pred"] = numeric(g["pred"])
        ...
        if "dataset_version" in g.columns:
            ...
            req = norm(DIAG_DATASET_VERSION)
            mask &= ds.eq(req) | ds.str.contains(req, na=False)
        available_site_cols = [c for c in site_cols if c in g.columns]
        if available_site_cols:
            sm = np.zeros(len(g), dtype=bool)
            for c in available_site_cols:
                sm |= g[c].astype(str).str.contains(
                    site, case=False, na=False, regex=False
                ).to_numpy()
            mask &= sm
        return g.loc[mask].dropna(subset=["time", "height_m", "pred"]).copy(), table

    try:
        gu, table = load_site_rows("U", DIAG_U_METHOD)
        report["u_csv"] = str(table)
        for hval, gh in gu.groupby("height_m"):
            if not np.isfinite(hval):
                continue
            h = int(round(float(hval)))
            value_name = f"__diag_u_{h}"
            out, align = merge_diagnostic_height_by_time(
                out, gh[["time", "pred"]], value_name
            )
            accepted = float(align["coverage_after_merge"]) >= float(DIAG_MIN_VALID_RATIO)
            ref = numeric(out[value_name]) if accepted else pd.Series(np.nan, index=out.index)
            u_col = f"U_MO{h} (m/s)"
            ...
            out[most_col] = ref
            out[keep_col] = ref
            wrf_col = first_existing(out, wrf_ws_candidates(h))
            if wrf_col is not None:
                out[diff_col] = ref - numeric(out[wrf_col])
            align.update({
                "height_m": h, "accepted": bool(accepted),
                "valid": int(ref.notna().sum()), "wrf_ws_col": wrf_col,
            })
            report["u_heights"].append(align)
            out = out.drop(columns=[value_name])
        report["u_status"] = "injected" if report["u_heights"] else "no_site_rows"
    except Exception as exc:
        report["u_status"] = "failed"
        report["u_error"] = repr(exc)

    try:
        gt, table = load_site_rows("T", DIAG_T_METHOD)
        report["t_csv"] = str(table)
        for hval, gh in gt.groupby("height_m"):
            if not np.isfinite(hval):
                continue
            h = int(round(float(hval)))
            value_name = f"__diag_t_{h}"
            out, align = merge_diagnostic_height_by_time(
                out, gh[["time", "pred"]], value_name
            )
            accepted = float(align["coverage_after_merge"]) >= float(DIAG_MIN_VALID_RATIO)
            ...
            diff_col = f"T_MO_minus_WRF{h} (K)"
            most_diff_col = f"MOST_T_minus_WRF{h} (K)"
            keep_col = f"GryningThermal_T_MO{h} (K)"
            out[t_col] = ref
            out[most_t_col] = ref
            out[keep_col] = ref
            wrf_col = first_existing(out, wrf_temp_candidates(h))
            if wrf_col is not None:
                out[diff_col] = ref - numeric(out[wrf_col])
                out[most_diff_col] = out[diff_col]
            align.update({
                "height_m": h, "accepted": bool(accepted),
                "valid": int(ref.notna().sum()), "wrf_t_col": wrf_col,
            })
            report["t_heights"].append(align)
            out = out.drop(columns=[value_name])
        report["t_status"] = "injected" if report["t_heights"] else "no_site_rows"
    except Exception as exc:
        report["t_status"] = "failed"
        report["t_error"] = repr(exc)
...
def align_records(
    ...
    """Align saved heads to Xilinhaote target columns.

    Xilinhaote Code2 normally evaluates WD at 10, 30, 70 and 100 m, while the
    training drop rule removes WD heads at 4 and 50 m. A saved WD head remains
    at its physical height whenever target OBS and WRF exist there. Only a
    non-evaluable saved WD head is rebased to the target tower's configured
    native 10-m WD baseline. Target observations are never interpolated.
    """
    out = df.copy()
...
def generate_tasks() -> List[GeneralizationTask]:
    """Generate every ordered cross-tower pair among the four Xilinhaote sites."""
        ...
        for source_site in SOURCE_SITES:
            for target_site in TARGET_SITES:
                ...
                tasks.append(GeneralizationTask(
                    f"{source_site}_to_{target_site}_same_time",
                    "cross_tower_same_time",
                    source_site,
                    target_site,
                    "source_test_common_time",
                ))
        ...
        for source_site in SOURCE_SITES:
            for target_site in TARGET_SITES:
                if source_site == target_site:
                    continue
                tasks.append(GeneralizationTask(
                    f"{source_site}_to_{target_site}_all_target_time",
...
def apply_model_task(
    ...
    df, diagnostic_report = inject_diagnostic_references(target_raw, task.target_site)
    ...
    audit: Dict[str, Any] = {
        ...
        "task": task.__dict__, "diagnostic_references": diagnostic_report,
        ...
        "target_native_wd_heights_m": {site: int(cfg["native_wd_m"]) for site, cfg in SITE_CONFIG.items()},
...
def main() -> None:
    ...
    summary = {
        ...
        "scope": "Four-site ordered-pair cross-tower same-time plus all-valid-target-time generalization (12 pairs per time mode).",
