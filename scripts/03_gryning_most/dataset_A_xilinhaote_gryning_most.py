...
# -*- coding: utf-8 -*-
"""
Gryning / MOST full profile diagnostics for xilinhaote.
...
CASE_NAME = "xilinhaote"
TIME_COL = "北京时间"
ML_READY_ROOT = Path(r"D:\wake\WRF_RANS_wake\xilinhaote\result\bias_correction\1km\inputdata\data_post_new\four_sites_full")
ML_READY_GLOB = r"**/ML_ready_ref10_*_d03.csv"
OUTPUT_ROOT = Path(r"D:\wake\WRF_RANS_wake\xilinhaote\result\bias_correction\1km\inputdata\data_post_new\gryning_diagnostics")
...
def safe_plot_path(path: Path, max_chars: int = 235) -> Path:
    """Return a Windows-safe shorter PNG path when a long filename may exceed MAX_PATH.

    On legacy Windows setups, PIL/matplotlib may raise FileNotFoundError when the
    full filename approaches ~260 characters even if the parent folder exists.
    This keeps the directory layout but shortens only the plot basename, adding a
    stable hash so names remain unique.
    """
    path = Path(path)
    ...
    if len(s) <= int(max_chars):
        return path
    digest = hashlib.md5(path.stem.encode("utf-8", errors="ignore")).hexdigest()[:8]
    suffix = path.suffix or ".png"
    parent_len = len(str(path.parent)) + 1
    # Reserve room for underscore, hash, and suffix.
    keep = int(max_chars) - parent_len - len(suffix) - len(digest) - 1
    keep = max(24, keep)
    short_stem = f"{path.stem[:keep]}_{digest}"
    return path.parent / f"{short_stem}{suffix}"


def savefig_safe(fig, out_png: Path, dpi: int = None) -> Path:
    out_png = safe_plot_path(Path(out_png))
    fig.savefig(out_png, dpi=PLOT_DPI if dpi is None else dpi)
    return out_png
...
def plot_timeseries(long_df: pd.DataFrame, out_root: Path, variable: str, selected_methods: List[str]) -> None:
    ...
    for keys, g0 in long_df.groupby(["relative_file", "site_id", "dataset_version", "height_m"], dropna=False):
        ...
        if g0.empty:
            continue
        stem = f"{safe_name(dataset_version, 12)}__{safe_name(site_id, 14)}__{variable}_{float(h):g}m"
        ...
        fig.autofmt_xdate()
        fig.tight_layout()
        savefig_safe(fig, out_root / f"{stem}_timeseries.png")
        plt.close(fig)
        ...
        fig.autofmt_xdate()
        fig.tight_layout()
        savefig_safe(fig, out_root / f"{stem}_error_timeseries.png")
        plt.close(fig)
...
def plot_summary_bars(summary: pd.DataFrame, out_root: Path, variable: str) -> None:
    ...
    for metric in ["RMSE", "MAE", "bias", "Pearson_r"]:
        ...
        fig.tight_layout()
        savefig_safe(fig, out_root / f"{variable}_{metric}_resolution_method_bar.png")
        plt.close(fig)
    ...
    if SAVE_PLOTS and plt is not None:
        ...
        for keys, g in u_metrics.groupby(["relative_file", "site_id", "dataset_version"], dropna=False):
            ...
            if PLOT_METRIC_BY_HEIGHT_DATASET_VERSIONS and version not in PLOT_METRIC_BY_HEIGHT_DATASET_VERSIONS:
                continue
            stem = f"{safe_name(version, 12)}__{safe_name(site, 14)}"
