...
ML_MERGE_TOLERANCE = "20min"

NEST_LEVEL = 3

WRF_OUTPUT_DIR = r"/data/home/maguolin/ask/wake/rans/xilinhaote/meso_1km"
WRFOUT_DIR = r"/data/home/maguolin/ask/wake/rans/xilinhaote/meso_1km"
OUTPUT_DIR = r"/data/home/maguolin/ask/wake/rans/xilinhaote/meso_1km/data_post_new/four_sites_full"

AVG_START_STR = "2009-05-01 00:00"
AVG_END_STR   = "2010-07-01 00:00"
...
STATIONS = [
    {
        "station_name": "t001",
        "site_tag": "NO1_1159",
        "obs_csv": r"/data/home/maguolin/ask/wake/rans/xilinhaote/meso/data_post/obs/NO1_116.297_44.124_1159_result.csv",
    },
    {
        "station_name": "t002",
        "site_tag": "NO2_1107",
        "obs_csv": r"/data/home/maguolin/ask/wake/rans/xilinhaote/meso/data_post/obs/NO2_116.3467_44.193_1107.csv",
    },
    {
        "station_name": "t003",
        "site_tag": "NO3_1071",
        "obs_csv": r"/data/home/maguolin/ask/wake/rans/xilinhaote/meso/data_post/obs/NO3_116.3515_44.16375_1071.csv",
    },
    {
        "station_name": "t004",
        "site_tag": "NOT_1166",
        "obs_csv": r"/data/home/maguolin/ask/wake/rans/xilinhaote/meso/data_post/obs/NOT_100_result_new.csv",
    },
]
...
def clean_obs_dataframe(df, csv_file=None, verbose=True):
    ...
    for c in df.columns:
        ...
        prefix = None
        for cand in ["wspd", "wdir", "tp", "rh", "tbl"]:
...
def build_obs_period_profiles(csv_file, avg_start, avg_end, instrument_name=None):
    ...
    common_hs = sorted(set(wspd_hs.keys()).intersection(tbl_hs.keys()))
    for h in common_hs:
...
def _extract_obs_profile_wide(obs_csv, heights):
    ...
    for h in sorted(set(float(x) for x in heights)):
        ...
        if has_ws and has_ti:
            ...
            # Standard alias expected by Code 1-4. The original _from_TI column is kept.
...
def _merge_asof_on_time_ns(left_df, right_df, left_time_col, right_time_col, tolerance="20min", right_keep_cols=None):
...
def merge_obs_wrf_timeseries(obs_ts, wrf_ts, obs_col, wrf_col, tolerance="20min"):
...
def merge_obs_wrf_scalar_timeseries(obs_ts, wrf_ts, obs_col, wrf_col, tolerance="20min"):
    if obs_ts is None or obs_ts.empty or wrf_ts is None or wrf_ts.empty:
        return pd.DataFrame()
...
def process_monitoring_data(
    ...
    for iw in range(num_windows):
        ...
        window_end_time = beijing_times[end_idx - 1]
...
if __name__ == "__main__":
    ...
    print("=" * 90)
    print(f"开始批量处理 {len(STATIONS)} 个站点, 嵌套层: d{NEST_LEVEL:02d}")
    ...
    for site in STATIONS:
        ...
        print("\n" + "=" * 90)
        print(f"开始处理站点: {site_tag} ({station_name})")
        print("=" * 90)
        ...
        try:
            ...
            if avg_start is not None and avg_end is not None and (result_df is not None) and (not result_df.empty):
                Ri_mean, L_mean = plot_Ri_L_timeseries(
                    ...
                    avg_end,
                    output_dir=site_output_dir,
                    fig_name=f"Ri_L_timeseries_{site_tag}_d{NEST_LEVEL:02d}.png"
                )
                mean_PBLH = plot_PBLH_timeseries(
                    ...
                    avg_end,
                    output_dir=site_output_dir,
                    fig_name=f"PBLH_timeseries_{site_tag}_d{NEST_LEVEL:02d}.png"
                )

                mean_txt = os.path.join(site_output_dir, f"Ri_L_mean_{site_tag}_d{NEST_LEVEL:02d}.txt")
            ...
            if wrf_profile_map.get("wspd") is not None:
                plot_obs_vs_wrf_profile_points(
                    ...
                    output_dir=site_output_dir,
                    fig_name=f"WRF_vs_obs_ws_profile_{site_tag}_d{NEST_LEVEL:02d}_full.png",
                ...
                plot_obs_vs_wrf_profile_points(
                    ...
                    obs_x_col=OBS_VAR_META["wspd"]["obs_col"],
                    output_dir=site_output_dir,
                    fig_name=f"WRF_vs_obs_ws_profile_{site_tag}_d{NEST_LEVEL:02d}_0-{int(PROFILE_ZOOM_MAX)}m.png",
                    xlabel="Wind speed (m s$^{-1}$)",
                    title="WRF vs Observations (Wind-speed profile)",
            ...
            if wrf_profile_map.get("wdir") is not None:
                plot_obs_vs_wrf_profile_points(
                    ...
                    output_dir=site_output_dir,
                    fig_name=f"WRF_vs_obs_wd_profile_{site_tag}_d{NEST_LEVEL:02d}_full.png",
                ...
                plot_obs_vs_wrf_profile_points(
                    ...
                    obs_x_col=OBS_VAR_META["wdir"]["obs_col"],
                    output_dir=site_output_dir,
                    fig_name=f"WRF_vs_obs_wd_profile_{site_tag}_d{NEST_LEVEL:02d}_0-{int(PROFILE_ZOOM_MAX)}m.png",
                    xlabel="Wind direction (deg)",
                    title="WRF vs Observations (Wind-direction profile)",
                    zmax=PROFILE_ZOOM_MAX
                )
            ...
            if wrf_profile_map.get("tp") is not None:
                plot_obs_vs_wrf_profile_points(
                    ...
                    output_dir=site_output_dir,
                    fig_name=f"WRF_vs_obs_tp_profile_{site_tag}_d{NEST_LEVEL:02d}_full.png",
                ...
                plot_obs_vs_wrf_profile_points(
                    ...
                    obs_x_col=OBS_VAR_META["tp"]["obs_col"],
                    output_dir=site_output_dir,
                    fig_name=f"WRF_vs_obs_tp_profile_{site_tag}_d{NEST_LEVEL:02d}_0-{int(PROFILE_ZOOM_MAX)}m.png",
                    xlabel="Temperature (°C)",
                    title="WRF vs Observations (Temperature profile)",
                    zmax=PROFILE_ZOOM_MAX
                )
            ...
            if wrf_profile_map.get("tke") is not None:
                plot_obs_vs_wrf_profile_points(
                    ...
                    output_dir=site_output_dir,
                    fig_name=f"WRF_vs_obs_tke_profile_{site_tag}_d{NEST_LEVEL:02d}_full.png",
                    xlabel="TKE (m$^2$ s$^{-2}$)",
                ...
                plot_obs_vs_wrf_profile_points(
                    ...
                    obs_x_col=OBS_VAR_META["tke"]["obs_col"],
                    output_dir=site_output_dir,
                    fig_name=f"WRF_vs_obs_tke_profile_{site_tag}_d{NEST_LEVEL:02d}_0-{int(PROFILE_ZOOM_MAX)}m.png",
                    xlabel="TKE (m$^2$ s$^{-2}$)",
                    title="WRF vs Observations (TKE profile)",
                    zmax=PROFILE_ZOOM_MAX
                )
            ...
            if avg_start is not None and avg_end is not None and native_period_df is not None:
                plot_wrf_only_profiles(
                    native_period_df,
                    output_dir=site_output_dir,
                    prefix=f"WRF_profile_{site_tag}_d{NEST_LEVEL:02d}",
                    zmax=PROFILE_ZOOM_MAX
                )
                ...
                if native_tke_period_df is not None:
                    plot_native_tke_profile(
                        ...
                        output_dir=site_output_dir,
                        fig_name=f"TKE_profile_{site_tag}_d{NEST_LEVEL:02d}.png",
                        zmax=PROFILE_ZOOM_MAX
                    )
            ...
            if avg_start is not None and avg_end is not None and result_df is not None and not result_df.empty:
                ...
                ws_compare = merge_obs_wrf_timeseries(obs_ws_ts, wrf_ws_ts, obs_col=OBS_VAR_META["wspd"]["obs_col"], wrf_col="十分钟平均风速 (m/s)", tolerance="20min")
                wd_compare = merge_obs_wrf_timeseries(obs_wd_ts, wrf_wd_ts, obs_col=OBS_VAR_META["wdir"]["obs_col"], wrf_col="十分钟平均风向 (°)", tolerance="20min")
                tp_compare = merge_obs_wrf_timeseries(obs_tp_ts, wrf_tp_ts, obs_col=OBS_VAR_META["tp"]["obs_col"], wrf_col="十分钟平均温度 (°C)", tolerance="20min")
                tke_compare = merge_obs_wrf_timeseries(obs_tke_ts, wrf_tke_ts, obs_col=OBS_VAR_META["tke"]["obs_col"], wrf_col="TKE (m2/s2)", tolerance="20min") if not wrf_tke_ts.empty else pd.DataFrame()
                l_compare = merge_obs_wrf_scalar_timeseries(obs_l_ts, wrf_l_ts, obs_col=OBS_VAR_META["l"]["obs_col"], wrf_col="L (m)", tolerance="20min")
                ...
                export_map = {
                    f"full_timeseries_ws_{site_tag}_d{NEST_LEVEL:02d}.csv": full_ws,
                    f"matched_timeseries_ws_{site_tag}_d{NEST_LEVEL:02d}.csv": matched_ws,
                    f"full_timeseries_wd_{site_tag}_d{NEST_LEVEL:02d}.csv": full_wd,
                    f"matched_timeseries_wd_{site_tag}_d{NEST_LEVEL:02d}.csv": matched_wd,
                    f"full_timeseries_L_{site_tag}_d{NEST_LEVEL:02d}.csv": full_L,
                    f"matched_timeseries_L_{site_tag}_d{NEST_LEVEL:02d}.csv": matched_L,
                }
                ...
                if not full_ws.empty:
                    plot_timeseries_comparison_dual(
                        ...
                        wrf_col="十分钟平均风速 (m/s)",
                        output_dir=site_output_dir,
                        fig_prefix=f"Timeseries_ws_compare_{site_tag}_d{NEST_LEVEL:02d}",
                ...
                if not full_wd.empty:
                    plot_timeseries_comparison_dual(
                        ...
                        wrf_col="十分钟平均风向 (°)",
                        output_dir=site_output_dir,
                        fig_prefix=f"Timeseries_wd_compare_{site_tag}_d{NEST_LEVEL:02d}",
                ...
                if not full_L.empty:
                    plot_scalar_timeseries_comparison_dual(
                        ...
                        wrf_col="L (m)",
                        output_dir=site_output_dir,
                        fig_name=f"Timeseries_L_compare_{site_tag}_d{NEST_LEVEL:02d}.png",
                ...
                if error_stats_tables:
                    ...
                    error_stats_all_csv = os.path.join(site_output_dir, f"error_stats_all_{site_tag}_d{NEST_LEVEL:02d}.csv")
                ...
                for key, (cmp_df, obs_c, wrf_c) in compare_pack.items():
                    ...
                    out_csv = os.path.join(site_output_dir, f"matched_profile_{key}_{site_tag}_d{NEST_LEVEL:02d}.csv")
